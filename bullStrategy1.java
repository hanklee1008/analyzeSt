...
public class bullStrategy1 {
				...
				if (content.size()>=quarterKCount)
				{
					if (isComputeReturn==0)
					{
						if(findEntryPoint(s,shfile,content,contemp,temp,kstate,entrypoint,buyday))
						{
							...
							tempdata=new String[]{stockname,buyday[kstate[2]],"",""+(currentHigh-testpoint)/content.get(content.size()-1)[3],""+test,"",quantity+"",df.format(weeklyrate),df.format(lead),"","","","","","","","","","1"};
	...
	{	
		test=(currentk[1]-currentk[0])/currentk[0];
				...
				if((enterPoint-compare[2])/compare[2]*100>=7)//眖翴衡>=7%
				{
					...
					else 
					{
						if(enterPoint >= compare[0])// k
							if (base.get(base.size() - 1)[3]>=base.get(base.size() - 1)[0])// 玡k
								if (base.get(base.size() - 1)[1] - compare[2] > compare[1]- base.get(base.size() - 1)[1])// k次辅玡k次柑
								{
									return 4;
								}
					}
	...
	private double computeEnterPoint(ArrayList<double[]> base,double[] compare)
	{
		...
		{
			if(isTurnQuarterLine(base,compare,keypoint))//﹗絬		
				if(isTurnMonthLine(base,compare,keypoint))//る絬	
	...
	{					
		if (low<returnv[1])
		{
			if ((returnv[0]-enterpoint)/enterpoint<0.07)//蔼翴ぃ禬筁7%
			{
				returnv[1]=low;		
			}				
		}
	...
	{
		if ((enterpoint-low)/enterpoint>0.13)//┰禬筁13%
		{
			return true;
		}
	...
	{
		if(curMonthline<preMonthline)//る絬
		{
			if ((low-enterpoint)/enterpoint>=0.1)//蔼翴害禬筁10%
			{
				return true;
			}							
		}
